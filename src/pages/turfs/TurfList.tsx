  ...
  if (isLoading) {
    return (
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 p-8">
        <h1 className="text-4xl col-span-full text-center">Loading...</h1>
      </div>
    );
  }
    ...
    <Card className="flex flex-col">
      ...
      <CardContent className="p-4 flex flex-col grow">
        ...
        <div className="mt-3 flex flex-wrap gap-2">
          {turf.amenities.slice(0, 3).map((amenity) => (
            <Badge key={amenity} variant="outline" className="text-xs px-2 py-1 rounded-full">
              {amenity}
            </Badge>
          ))}
        </div>
      ...
      <CardFooter className="flex justify-between items-center p-4">
        ...
        <Link to={`/turfs/${turf.slug}`} className="w-full md:w-auto">
          <Button className="w-full md:w-auto hover:cursor-pointer">View Details</Button>
        </Link>
      </CardFooter>
    </Card>
  );
